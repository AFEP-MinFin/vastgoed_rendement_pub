...
```{r}
sq_outcomes <- compare_rents_to_return_goal(list(name = 'SQ', returns = sq))

sq_outcomes$counts %>% group_by(wws_cat) %>% summarise(n = sum(n))
sq_outcomes$histogram
sq_outcomes$woz_plot
```

opt_outcomes\$histogram

opt_outcomes\$woz_plot
...
opt_outcomes <- compare_rents_to_return_goal(list(name = 'Opt', returns = opt))
opt_outcomes$counts %>% group_by(wws_cat) %>% summarise(n = sum(n))
opt_outcomes$histogram
opt_outcomes$woz_plot
```

```{r}
initial_guess <- c(bA1 = 1, bA91 = 11041, bA91b = 172, bA93i = 0.5)
...
ga_result <- ga(type = "real-valued",
                ...
                lower = c(0, 0, 0, 0,),  # Lower bounds for x and y
                upper = c(2, 20000, 350, 1),    # Upper bounds for x and y
